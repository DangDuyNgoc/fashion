...
export function DataTable<TData, TValue>({
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const ids = selectedRows.map((row) => (row.original as { id: string | number }).id);
      
      await Promise.all(ids.map((id) => productsService.delete(Number(id))));
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
          <div className="bg-background p-6 rounded-md shadow-lg max-w-sm w-full border">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Bạn có chắc chắn muốn xóa {table.getFilteredSelectedRowModel().rows.length} sản phẩm đã chọn? Hành động này không thể hoàn tác.
            </p>
      ...
      {Object.keys(rowSelection).length > 0 && (
        <div className="flex justify-end">
          ...
          >
            ...
            Xóa sản phẩm đã chọn ({Object.keys(rowSelection).length})
          </button>
        </div>
      )}
      <Table>
